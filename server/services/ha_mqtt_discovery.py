...
* **Commands:** ``<prefix>/<site>/<key>/set`` runs the descriptor's typed
  ...
  The site mode panel's ``site_mode/set`` needs ``settings.manage``.
...
class Topics:
    ...
    def state(self, key: str) -> str:
        return f"{self.prefix}/{self.site_slug}/{key}/state"

    def attributes(self, key: str) -> str:
        return f"{self.prefix}/{self.site_slug}/{key}/attributes"

    def command(self, key: str) -> str:
        return f"{self.prefix}/{self.site_slug}/{key}/set"

    def event(self, key: str) -> str:
        return f"{self.prefix}/{self.site_slug}/{key}/event"
...
def build_discovery(topics: Topics, version: str, descriptors: list[dict[str, Any]],
                    device_names: dict[str, str], *, site_mode: bool) -> dict[str, dict]:
...
class MqttBridge:
    ...
    def _principal(self, db):
        ...
        row = db.query(ApiToken).filter(ApiToken.id == self.settings.api_token_id).first()
    ...
    async def stop(self, *, clear: bool = False) -> None:
        """``clear``: also remove the discovery this bridge published, so its
        devices leave Home Assistant (the integration is gone, not restarting)."""
        self._clear = self._clear or clear
        self._stop.set()
        ...
        async with client(self.settings, identifier=f"opennvr-{topics.site_slug}-"
            ...
            try:
                ...
                with contextlib.suppress(Exception):
                    ...
                    if self._clear:
                        await self._clear_discovery(mq, topics)

    async def _clear_discovery(self, mq, topics: Topics) -> None:
        """An empty retained config removes a device from Home Assistant."""
        for key in list(self._published):
            await mq.publish(topics.device_config(key), None, qos=1, retain=True)
    ...
    async def _publish_all(self, mq, topics: Topics, snap: dict[str, Any], version: str,
        ...
        for key, payload in devices.items():
            now = set(payload["cmps"])
            gone = self._published.get(key, set()) - now
            for obj in gone:  # removal: the component with its platform only
                payload["cmps"][obj] = {"p": self._removed_platform(obj)}
            await mq.publish(topics.device_config(key), json.dumps(payload), qos=1,
                             retain=True)
    ...
    def _removed_platform(self, obj_id: str) -> str:
        key = obj_id.removeprefix("opennvr_")
        for k, platform in self._platform.items():
            if _slug(k) == key:
                return platform
        return "sensor"
        ...
        async for message in mq.messages:
            ...
            if topic == f"{self.settings.discovery_prefix}/status":
                if payload.strip().lower() == "online":
                    ...
                    await self._publish_all(mq, topics, snap, server_version())
                continue
            key = topics.key_of_command(topic)
            if key is not None:
                await self._command(mq, topics, key, payload[:256])

    async def _command(self, mq, topics: Topics, key: str, payload: str) -> None:
        ...
        try:
            with SessionLocal() as db:
                ...
                if key == SITE_MODE_KEY:
                    detail = await self._set_site_mode(db, principal, payload)
                else:
    ...
    async def _set_site_mode(self, db, principal, payload: str) -> dict[str, Any]:
        from fastapi import HTTPException
