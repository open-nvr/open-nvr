...
"""Run a server-described entity's command (HA-114), for every way in.

``POST /entities/{key}/command`` (routers/entities.py) and the MQTT bridge's
``.../set`` topics (services/ha_mqtt_discovery.py) both come here, so a
command is checked the same way whoever sends it: only a descriptor the
principal can SEE can be commanded (its required_scope and camera decide
that), the command is typed, never a URL, and each control re-checks what
it touches. Errors are HTTPExceptions; callers outside HTTP map them.
"""
...
class CommandIn:
    """A command's value and arguments (the HTTP body's shape)."""

    def __init__(self, value: Any = None, args: dict[str, Any] | None = None) -> None:
        self.value = value
        self.args = dict(args or {})


async def run_command(db: Session, principal, key: str, value: Any = None,
                      args: dict[str, Any] | None = None) -> tuple[ed.Descriptor, Any]:
    """``(descriptor, result)``; 404 when the principal can't see ``key`` or
    it has no command."""
    desc = next((d for d in ed.descriptors_for(db, principal) if d.key == key), None)
    if desc is None or not desc.command:
        raise HTTPException(status_code=404, detail="No such entity command")
    body = CommandIn(value, args)
    kind = desc.command.get("type")
    if kind == "core_control":
        return desc, await _core_control(db, principal, desc, body)
    if kind == "app_action":
        return desc, await _app_action(db, principal, desc, body)
...
async def _core_control(db: Session, principal, desc: ed.Descriptor,
                        body: CommandIn) -> dict:
