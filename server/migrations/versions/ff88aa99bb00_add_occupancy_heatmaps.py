...
"""add occupancy_heatmaps + occupancy_footfall (heatmap / footfall history)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("dwell_count", sa.Integer(), nullable=False, server_default="0"),
        sa.Column("dwell_seconds", sa.Float(), nullable=False, server_default="0"),
        sa.Column("dwell_max_seconds", sa.Float(), nullable=False,
                  server_default="0"),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
    )
    op.create_index(
        "ix_occupancy_footfall_camera_hour", "occupancy_footfall",
        ["camera_id", "hour_start"], unique=True,
    )


def downgrade() -> None:
    op.drop_index("ix_occupancy_footfall_camera_hour",
                  table_name="occupancy_footfall")
    op.drop_table("occupancy_footfall")
