...
morning?") and what the ``opennvr.search_events`` service answers. Structured
filters only in v1: ``q`` matches labels and plates on events, and title,
description and source on alerts. Semantic ``q`` through a KAI-C embedding
adapter is HA-502 (M5); the response says ``semantic: false`` until then.
...
from models import AppAlert, CameraZone, TimelineEvent, User
...
async def search(
    ...
    """Newest first. Each result has ``kind`` (event|alert), ``at`` and
    ``camera_id`` plus its own fields; events carry ``evidence_url`` (sign it
    with ``POST /media/sign`` for a phone)."""
    ...
    return {"results": results[:limit], "semantic": False,
