...
class AppConfig:
    ...
    # serves https directly (the compose overlay mounts a self-signed pair
    # from agent-certs/, generated by the certs-init one-shot — same pattern
    # as mediamtx-certs). Empty/None → plain http (loopback dev). getUserMedia
    # (the demo mic) requires a secure origin, so LAN voice needs this on.
...
SKILL_TOOLS: dict[str, list[str]] = {
    ...
    "events": ["recent_events", "recent_plates", "search_history",
               "describe_event", "describe_window"],
...
def load_config(path: str | Path) -> AppConfig:
    ...
    if _tls_cert and not (Path(_tls_cert).is_file() and Path(_tls_key).is_file()):
        raise SystemExit(f"config: TLS cert/key not found: {_tls_cert} / {_tls_key}")
    _listen_host = str(raw.get("listen_host") or raw.get("host") or "127.0.0.1")
    if _auth_mode == "none" and _listen_host not in ("127.0.0.1", "localhost", "::1"):
        logger.warning(
            "config: auth_mode is 'none' but the agent listens on %s — every "
            "device that can reach this port can query your cameras. Set "
            "auth_mode: opennvr for any non-loopback deployment.", _listen_host)
    ...
    return AppConfig(
        ...
        port=_int("port", 9100),
        tls_certfile=_tls_cert,
        tls_keyfile=_tls_key,
...
class CameraAgentRuntime:
    ...
    def __init__(self, cfg: AppConfig) -> None:
        ...
        self.tool_handlers = {
            ...
            "recent_plates": self.tools.recent_plates,
...
def build_app(runtime: CameraAgentRuntime) -> FastAPI:
    ...
    async def _auth_gate(request: Request, call_next):
        ...
        if path in _OPEN_PATHS or path == "/demo" or path.startswith("/demo/"):
            return await call_next(request)
        # Device firewall first (mirrors the server's device-before-auth
        # order, so a blocked browser can't probe credential validity).
        # Header from the demo page; cookie fallback mirrors the server's
        # token_from_request for browsers sharing the OpenNVR origin.
        dev_tok = (request.headers.get("x-device-token")
                   or request.cookies.get("opennvr_device"))
        if runtime.auth and not await runtime.auth.device_allowed(dev_tok):
            return JSONResponse({"error": "device_not_approved"},
                                status_code=403)
    ...
    async def _auth_login(request: Request) -> JSONResponse:
        ...
        status, data = await runtime.auth.login(
            ...
            # Forward the browser's device identity so the server's device
            # firewall re-recognises it instead of minting a new device row
            # per login, and the admin's device list shows the real UA.
    ...
    async def _ws(websocket: WebSocket) -> None:
        ...
        if runtime.cfg.auth_mode == "opennvr":
            # Device firewall first (parity with the HTTP gate); the client
            # passes its device identity as ?device_token=.
            _dev = websocket.query_params.get("device_token") or None
            if runtime.auth and not await runtime.auth.device_allowed(_dev):
                await websocket.close(code=4403)   # 4403 = device blocked
                return
...
class _RedactTokensFilter(logging.Filter):
    ...
    _SENSITIVE = re.compile(
        # ``device_token`` needs its own alternative: ``\btoken`` cannot
        # match inside it (the ``_`` is a word character, so no boundary).
        r"\b(jwt|device_token|token|access_token|refresh_token|api_key)=[^&\s\"']+",
        re.IGNORECASE)
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = uvicorn.Config(
        ...
        # TLS (LAN exposure): serve https directly when a cert pair is
        # configured (agent-certs volume in the compose overlay). Also what
        # makes the mic work from LAN devices — getUserMedia needs a secure
        # origin.
